...
class SwapRoomEmulatorService {

  /**
   *
   * @param options
   * @param {string} options.address - websocket address
   */
  constructor(options) {
    this.events   = new Events()
    this.opts     = options
  ...
  init() {
    this.ws = new Ws(this.opts.address || 'ws://127.0.0.1:3000')

    this.ws.on('connect', this._init)
  }

  _init = () => {
    this.peer = this.ws.peer
...
export default SwapRoomEmulatorService
